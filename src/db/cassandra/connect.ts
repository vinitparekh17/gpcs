...
class SigV4Authenticator implements auth.Authenticator {
  ...
  (signature: string): void => {
    const payload =
      `signature=${signature},access_key=${this.accessKeyId},amzdate=${timestamp}`;
  ...
  evaluateChallenge = (
    ...
    const digest = createHash("sha256").update(canonicalRequest, "utf-8").digest().toString("hex");

    const stringToSign =
      `${SigV4Authenticator.AWS4_SIGNING_ALGORITHM}\n${timestamp}\n${scope}\n${digest}`;
...
export const cassandraClient = NODE_ENV !== "development"
  ? new Client({
