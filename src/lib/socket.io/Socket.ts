...
export class SocketServer {
  ...
  private async generateAndEmitAudioStream(prompt: string, response: string): Promise<void> {
    try {
      const pollyResponse = await pollyClient.send(pollyCommand(response));
  ...
  private handleStreamingResponse(response: string, isCompleted: boolean): void {
    if (response && response !== '' && response !== 'SAFETY') {
      this.safeEmit(SocketEvents.RESPONSE_TEXT_STREAM, {
        response,
        isCompleted,
